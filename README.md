...
**`airflow-init` fails with `getpwuid(): uid not found`**

Your `.env` has `AIRFLOW_UID` set to a host uid that has no entry in the image's
`/etc/passwd`. The `airflow-init` service overrides the image entrypoint, which is
the thing that would normally register an arbitrary uid, so Airflow dies looking up
its own username. On macOS/Windows set `AIRFLOW_UID=50000`; the host-uid trick is a
Linux-only fix for bind-mount file ownership.

If you genuinely need a non-default `AIRFLOW_UID` (i.e. on Linux), fix the root
cause instead: drop `entrypoint: /bin/bash` from the `airflow-init` service so the
image's own entrypoint still runs, and express the init steps as the command —
`command: ["bash", "-c", "airflow db migrate && airflow users create ..."]`.

**DAG fails with a permission error on `/var/run/docker.sock`**

The scheduler's `group_add` doesn't match the socket's group. On Linux set
`DOCKER_GID` in `.env` to `stat -c '%g' /var/run/docker.sock` and recreate the
containers.

**`run_spark_etl` fails with `StatusCode: 255` and a `spark-submit` usage banner**

The DockerOperator's `command` must be a **single-element list**, not a string.
This image's entrypoint ends with `eval "$1"`, so it runs only the first argument;
a string gets split into separate argv entries and `spark-submit` receives no
script. Correct form:
...
**Jupyter or `spark-sql` can't find `demo.sales.orders`**

The interactive container's built-in `demo` catalog points at the wrong hostname.
See the caveat under "Inspect the results" and pass the catalog config explicitly.

**DAG doesn't appear in the UI**

Confirm the file is in `dags/` and check the scheduler's parse errors:
...
docker compose down -v       # also delete MinIO data, Postgres DB, notebooks
```
